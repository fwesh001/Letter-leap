...
(function () {
  try {
    ...
    if (winnerEl) {
      ...
      winnerEl.innerHTML = `🏆 <strong>${escapeHtml(winner || 'Winner')}</strong> wins! Reason: ${escapeHtml(reasonText)}`;
    }
    ...
    if (container) {
      ...
      players.forEach((p, idx) => {
        ...
        card.innerHTML = `
          <h3>${escapeHtml(p.name)} ${idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : ''}</h3>
          <ul>
    ...
    if (badgeList) {
      ...
      if (players.length) {
        addBadge('🏆', 'Top Scorer', `${players[0].name} scored ${players[0].score} pts`);
      }
      ...
      if (wordChain.length) {
        ...
        if (longestEntry && longestEntry.word) {
          ...
          addBadge('📖', 'Dictionary Expert', `${playerName} played "${longestEntry.word}" (${longestEntry.word.length} letters)`);
        }
      }
      ...
      if (players.length) {
        ...
        addBadge('👑', 'Accuracy Master', `${accuracyWinner.name} achieved ${accuracyWinner.accuracy} accuracy`);
      }

      // Iron Fingers ⌨️ - longest streak without messing up
      if (players.length) {
        const streakWinner = players.reduce((best, current) => 
          current.maxStreak > best.maxStreak ? current : best
        );
        if (streakWinner.maxStreak > 0) {
          addBadge('⌨️', 'Iron Fingers', `${streakWinner.name} had a ${streakWinner.maxStreak}-word streak`);
        }
      }
      ...
      if (players.length) {
        ...
        if (fastestPlayer.totalWords > 0) {
          addBadge('⚡', 'Fastest Fingers', `${fastestPlayer.name} played ${fastestPlayer.totalWords} words`);
        }
      }
      ...
      if (players.length && wordChain.length) {
        ...
        if (rareEntries.length > 0) {
          ...
          addBadge('🐉', 'Trap Master', `${trapMaster[0]} used ${trapMaster[1]} rare-letter words`);
        }
      }
    }
