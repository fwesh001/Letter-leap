...
fetch('../data/words.txt')
  ...
  .catch(err => {
    ...
    showPopup('Failed to load words list. Game cannot start.');
  });
...
function playClickSound() {
  ...
  if (clickSound) {
    ...
    clickSound.play().catch(err => console.warn('[sm-normal] click sound play failed:', err));
  }
}
...
function playCorrectSound() {
  ...
  if (correctSound) {
    ...
    correctSound.play().catch(err => console.warn('[sm-normal] correct sound play failed:', err));
  }
}
...
function playWrongSound() {
  ...
  if (wrongSound) {
    ...
    wrongSound.play().catch(err => console.warn('[sm-normal] wrong sound play failed:', err));
  }
}
...
function playGameoverSound() {
  ...
  if (gameoverSound) {
    ...
    gameoverSound.play().catch(err => console.warn('[sm-normal] gameover sound play failed:', err));
  }
}
...
function updateTimerDisplay() {
  ...
  timerElement.style.color = timeLeft > 20 ? 'green' : timeLeft > 10 ? 'yellow' : 'red';
}
...
function handleSubmission() {
  ...
  setTimeout(() => {
    ...
    if (!aiWord) {
      showPopup("You Win! AI could not find a word.", 1000);
      endGame(); return;
    }
