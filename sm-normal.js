...
fetch('words.txt')
  ...
  .catch(err => {
    ...
    showPopup('⚠️ Failed to load words list. Game cannot start.', 2500);
  });
...
function handleSubmission() {
  ...
  if (totalWordsExchanged === 5 || (totalWordsExchanged > 5 && totalWordsExchanged % 5 === 0)) {
    minWordLength++;
...
setTimeout(() => {
  ...
  if (!aiWord) {
    showPopup("🎉 You Win! AI couldn't find a word.", 2500);
    endGame();
    return;
  }
  ...
  updateGame();
  updateTimerDisplay();
}, 2500); // AI 'thinks' for 2.5 seconds
}
