...
fetch('words.txt')
  ...
  .catch(err => {
    ...
    showPopup('⚠️ Failed to load words list. Game cannot start.', 1000);
  });
...
function handleSubmission() {
  ...
  if (totalWordsExchanged === 3 || (totalWordsExchanged > 3 && totalWordsExchanged % 10 === 0)) {
    minWordLength++;
...
setTimeout(() => {
  ...
  if (totalWordsExchanged === 3 || (totalWordsExchanged > 3 && totalWordsExchanged % 3 === 0)) {
    minWordLength++;
    showPopup(`🎉 Minimum word length now set to ${minWordLength}!`);
  }
